...
import { ImageIcon, Plus, Search, Calendar, MoreHorizontal } from "lucide-react"
...
export default function Carousels() {
  ...
  useEffect(() => {
    ...
    if (storedProjects) {
      ...
      const carouselProjects = allProjects.filter((project: Project) => project.type === "carousel")
  ...
  const handleDelete = (id: string) => {
    ...
    if (storedProjects) {
      ...
      // Update state
      const updatedCarousels = projects.filter((project) => project.id !== id)
      setProjects(updatedCarousels)

      toast({
        title: "Deleted",
        description: "Carousel has been deleted.",
      })
    }
  }
    ...
    <div>
      ...
      {filteredProjects.length > 0 ? (
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => router.push(`/dashboard/edit/carousel/${project.id}`)}>
                          Edit
                        </DropdownMenuItem>
        ...
        <div className="flex flex-col items-center justify-center rounded-md border border-dashed py-12 text-center">
          ...
          <h3 className="mb-2 text-lg font-medium text-gray-900">No carousels found</h3>
          <p className="mb-4 max-w-md text-sm text-gray-600">
            {searchQuery ? "No carousels match your search criteria." : "You haven't created any carousels yet."}
          </p>
          <Link href="/dashboard/create/carousel">
            <Button className="bg-blue-600 hover:bg-blue-700">
              <Plus className="mr-2 h-4 w-4" />
